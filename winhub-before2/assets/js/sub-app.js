...
if (typeof gsap !== 'undefined') {
    // [MODIFIED] ScrollTrigger 플러그인 등록
...
// --- [ADDED] Decorative elements animation & parallax ---
/**
 * Mission 섹션의 장식용 사각형 요소에 애니메이션과 패럴랙스 효과를 설정합니다.
 * GSAP의 clip-path 속성을 사용하여 자연스러운 등장/퇴장 효과를 만듭니다.
 */
    ...
    if (rect1) {
        ...
        ScrollTrigger.create({
            ...
            onEnter: () => gsap.to(rect1, { 
                clipPath: 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)', // 펼치기
                duration: 1.2, 
                ease: 'expo.out' 
            }),
            onLeave: () => gsap.to(rect1, { 
                clipPath: 'polygon(100% 0%, 100% 0%, 100% 100%, 100% 100%)', // 오른쪽으로 접기
                duration: 0.8, 
                ease: 'power3.in' 
            }),
            onEnterBack: () => gsap.to(rect1, { 
                clipPath: 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)', // 다시 펼치기
                duration: 1.2, 
                ease: 'expo.out' 
            }),
            onLeaveBack: () => gsap.to(rect1, { 
                clipPath: 'polygon(0% 0%, 0% 0%, 0% 100%, 0% 100%)', // 왼쪽으로 다시 접기
                duration: 0.8, 
                ease: 'power3.in' 
            })
        });

        // rect1 내부 배경의 패럴랙스 효과
        if (parallaxBg) {
    ...
    if (rect2) {
        ...
        ScrollTrigger.create({
            ...
            onEnter: () => gsap.to(rect2, { 
                clipPath: 'polygon(100% 0%, 100% 100%, 0% 100%, 0% 0%)', // 펼치기
                duration: 1.2, 
                ease: 'expo.out'
            }),
            onLeave: () => gsap.to(rect2, { 
                clipPath: 'polygon(0% 0%, 100% 0%, 100% 0%, 0% 0%)', // 왼쪽 위로 접기
                duration: 0.8, 
                ease: 'power3.in' 
            }),
            onEnterBack: () => gsap.to(rect2, { 
                clipPath: 'polygon(100% 0%, 100% 100%, 0% 100%, 0% 0%)', // 다시 펼치기
                duration: 1.2, 
                ease: 'expo.out' 
            }),
            onLeaveBack: () => gsap.to(rect2, { 
                clipPath: 'polygon(100% 100%, 100% 100%, 0% 100%, 0% 100%)', // 오른쪽 아래로 다시 접기
                duration: 0.8, 
                ease: 'power3.in' 
            })
        });
...
function setupSubPageContentAnimations() {
    ...
    sections.forEach((section) => {
        // 원본 함수의 애니메이션 타겟 식별 로직을 유지하여 다른 페이지에서의 오작동을 방지합니다.
        let animatedElement;
        ...
        // contact.html 페이지의 특정 섹션에만 안정적인 애니메이션을 적용합니다.
        if (section.id === 'contact-section' || section.id === 'map-section') {
            // 이 섹션들은 한 번 나타난 후 사라지지 않아 키보드 문제로부터 안전합니다.
            ScrollTrigger.create({
                trigger: section,
                start: "top 85%", // 화면에 보이기 시작할 때
                toggleActions: 'play none none none', // 한 번만 재생하고 멈춤
                invalidateOnRefresh: true,
                onEnter: () => gsap.to(animatedElement, { opacity: 1, y: 0, duration: 0.8, ease: "power2.out" }),
            });
        } else {
            // 나머지 모든 섹션에는 기존의 등장/퇴장 애니메이션을 적용합니다.
            ScrollTrigger.create({
                trigger: section,
                start: "top 80%",
                end: "bottom 20%",
...
async function initializeSubpage() {
    ...
    // ▼▼▼ 애니메이션 설정 함수 호출 순서 수정 ▼▼▼
    setupSubPageContentAnimations(); // 범용 및 특정 섹션 애니메이션 처리
...
function handleSubPageResize() {
    // 너비가 변경되었을 때만 (예: 화면 회전) 파괴적인 재설정 로직을 실행합니다.
    ...
    // 너비 변경이 없는 경우 (가상 키보드 등장 등) 아무것도 하지 않습니다.
    // 이는 불필요한 refresh() 호출로 인한 스크롤 스냅과의 충돌을 방지하는 가장 안정적인 방법입니다.
}
