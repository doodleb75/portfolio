...
const getScaleConfig = (isDesktopView) => ({ hero: isDesktopView ? 2.5 : 1.6, part1: isDesktopView ? 3.0 : 2.0, part2: isDesktopView ? 3.0 : 2.0, part3: isDesktopView ? 3.2 : 2.2 });
...
const getTargetWinhubX = (isMobileView) => isMobileView ? 1.0 : 3.0;
const getTargetWinhubY = (isMobileView) => isMobileView ? -0.8 : -0.3;
...
async function runMainPageSequence() {
    ...
    try {
        /* [ROLLBACK COMMENTED OUT - OLD SPLINE CODE]
        ...
        */
        ...
        if (mainSplineApp) {
            ...
            console.error("MAIN-APP: GLTF Router Model could not be loaded.");
        }
    } catch (error) {
    ...
    if (headlineDivs.length > 0) {
        ...
        headlineDivs.forEach((divElement) => {
            gsap.set(divElement, { autoAlpha: 1, overflow: 'visible' });
            try {
                const lineSplit = new SplitText(divElement, { type: "chars", charsClass: "headline-char" });
                if (lineSplit.chars && lineSplit.chars.length > 0) {
                    splitHeadlineChars.push(lineSplit);
        ...
        splitHeadlineChars.forEach((lineSplit) => {
            if (lineSplit.chars && lineSplit.chars.length > 0) {
                lineSplit.chars.forEach((charEl) => {
                    ...
                    if (targetChar.trim() !== '') {
                        gsap.set(charEl, { autoAlpha: 0, x: 40 });
                        masterIntroTimeline.to(charEl, {
                            autoAlpha: 1,
                            x: 0,
                            duration: 0.7,
                            ease: "power2.out",
                        ...
                        }, `startHeadlineChars+=${globalCharOffset * 0.095}`);
                        globalCharOffset++;
                    } else {
...
function setupAllScrollTriggers(isDesktopView) {
    ...
    setupMainPageBackgroundChangeAnimations();
    if (mainSplineApp && winhub) setupSplineScrollAnimations(winhub, cable, isDesktopView);
    setupBarAnimations();
...
function setupResponsiveScrollTriggers() {
    ...
    // [ADDED] Debounced window resize listener to recalculate responsive coordinates
    // and rebuild GSAP scroll timelines after layout and Spline camera stabilize.
    let splineResizeTimeout;
    window.addEventListener('resize', () => {
        clearTimeout(splineResizeTimeout);
        splineResizeTimeout = setTimeout(() => {
            if (mainSplineApp && winhub) {
                const isDesktop = window.innerWidth >= 768;
                setupSplineScrollAnimations(winhub, cable, isDesktop);
                if (typeof ScrollTrigger !== 'undefined') {
