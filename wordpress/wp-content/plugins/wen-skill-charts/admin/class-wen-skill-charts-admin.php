...
function add_admin_form_scripts( $hook ) {
 ...
 /**
  ...
  public function get_transitions_options( $add_disable = true ) {
    ...
    $options = array(
      'left'   => __( 'Fade', 'ws-image-slider', 'wen-skill-charts' ),
      'middle' => __( 'Flip', 'ws-image-slider', 'wen-skill-charts' ),
      'right'  => __( 'Slide', 'ws-image-slider', 'wen-skill-charts' ),
    );
  ...
  function settings_meta_box_callback( $post ){
    ...
    <p><label><?php _e( 'Circle size', 'wen-skill-charts' ); ?>
      <input type="number" min="150" max="300" step="25" name="wen_skill_charts_settings[circle_size]" id="circle-size" value="<?php echo esc_attr( $settings_args['circle_size'] ); ?>" class="wsc-circle-size" data-default-color="<?php echo esc_attr( $defaults['circle_size'] ); ?>" required /></label>
