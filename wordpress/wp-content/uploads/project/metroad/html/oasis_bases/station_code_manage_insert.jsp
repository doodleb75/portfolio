<%@ page language="java"  pageEncoding="UTF-8" contentType="text/html;charset=UTF-8" %>
<%@ include file="../../inc/metro_dtype.jsp" %>
<body>

<div id="popBox" style="width:400px;">
	<!-- popupBox 헤더 -->
	<div class="pop_header">
		<h1>역사코드 등록</h1>
		<span class="p_close"><a href=""><img src="../../images/common/b_pop_close.png" alt="닫기버튼" title="닫기"/></a></span>
	</div>
	<!-- //popupBox 헤더 -->
	<!-- popupBox 컨텐츠 -->
	<div class="pop_body">
		<div class="station_code_regist">
			<p>
			<label for="호선">호선</label>
			<select name="" style="width:240px;">
				<option>3호선</option>
				<option>3호선</option>
				<option>3호선</option>
				<option>3호선</option>
			</select>
			</p>
			<p><label for="역사코드">역사코드</label> <input type="text" id="" name="" size="35"/></p>
			<p><label for="호선명칭">호선명칭</label> <input type="text" id="" name="" size="35"/></p>
		</div>
		
			<div class="Block_alignC">
				<span class="button small-g strong icon"><span class="confirm"></span><a href="#" title="닫기">저장</a></span>
				<span class="button small-g strong icon"><span class="cancel"></span><a href="#" title="닫기">닫기</a></span>
			</div>
	</div>
	<!-- //popupBox 컨텐츠 -->
</div>
</body>
</html>
